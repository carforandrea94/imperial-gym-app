...
Note sui valori:
- `opacity: 0.06` + `mix-blend-mode: soft-light`: le linee bianche restano
  appena percettibili, si fondono nel gradiente sottostante invece di
  sembrare un overlay piatto. Valore scelto dall'utente confrontando 3
  varianti (0.06/0.08/0.14) su un'anteprima visiva col gradiente reale —
  e' anche il parametro piu' facile da ritarare in futuro senza
  rigenerare l'SVG.
...
visibili ma sottili (non devono "rumoreggiare" sopra i contenuti), coerenti
con l'opacita' 0.06 gia' scelta e confermata dall'utente sull'anteprima,
devono muoversi in sincrono con il resto dello sfondo, e devono sparire
con `prefers-reduced-motion` attivo.
